...
├── backend/      # Backend API (Express + yt-dlp + FFmpeg + Node.js)
├── package.json  # Multi-platform root configuration & scripts
├── start.bat     # Windows 1-click launcher
├── start.ps1     # Windows PowerShell launcher
├── start.sh      # Linux / macOS launcher
├── install.bat   # Windows 1-click dependency installer
├── install.ps1   # Windows PowerShell dependency installer
...
1. Run `install.bat` to install all dependencies, `yt-dlp`, and check `ffmpeg`.
2. Double-click `start.bat` (or run `powershell ./start.ps1`) to start both servers.
...
- **User Workspace:** `http://localhost:5173/#/workspace`
